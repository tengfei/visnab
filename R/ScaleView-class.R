...
ScaleView.gen$methods(createView = function(seqname=NULL, rescale = "geometry"){
  ...
  castself <- function(){
   ...
   id.cf <- pars$xlimZoomChanged$connect(castself)
  ...
  pfunScale <- function(layer,painter,exposed){
    if(pars$geom == "twoside"){
      ...
      qdrawSegment(painter, pars$xlimZoom[1],-h/2+h/9+h/3,
                   pars$xlimZoom[1]+2*wids/5,-h/2+h/9+h/3, stroke = "black")
      qdrawSegment(painter, pars$xlimZoom[2],-h/2+h/9+h/3,
                   pars$xlimZoom[2]-2*wids/5,-h/2+h/9+h/3, stroke = "black")
      
      txts <- paste("Width:",as.integer(wids),"bp")
      qdrawText(painter, txts, cen, -h/2+h/9+h/3, color = "black")
      arrow.left <- qglyphArrow(direction="left")
      qdrawGlyph(painter, arrow.left, pars$xlimZoom[1],-h/2+h/9+h/3,
                 stroke = "black")
      arrow.left <- qglyphArrow(direction="right")
      qdrawGlyph(painter, arrow.left, pars$xlimZoom[2],-h/2+h/9+h/3,
                 stroke = "black")
  ...
  thisLayer <<- qlayer(rootLayer,paintFun=pfunScale,
                  ...
                  row=row, col=col, rowSpan=rowSpan, colSpan=colSpan)
  thisLayer$setLimits(qrect(0, h/2-h/9-3*h, pars$seqlength, h/2+h/9+h))
  pars$ylim <<- c(h/2-h/9-3*h, h/2+h/9+h)
