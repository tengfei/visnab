...
visnabGUIlist <- function(env){
  ...
  objlist <- as.list(env)
  N <- length(objlist)
  lapply(1:length(objlist),function(i){
      nrow <- (i-1)%/%4
      ncol <- i-1-nrow*4
      objlist[[i]]$view$show()
      layout$addWidget(objlist[[i]]$view,nrow,ncol)
  })
  Nrow <- N%/%4+1
  ## layout$addWidget(verticalSliders,1,0)
  layout$addWidget(slidersBox,Nrow,0,1,4)
  layout$setRowStretch(0,1)
  layout$setRowStretch(1,1)
  layout$setRowStretch(Nrow,0)
  grandWidget <- Qt$QWidget()
  grandWidget$setLayout(layout)
  grandWidget$setGeometry(0,0,800,900)
  ## obj$show()
